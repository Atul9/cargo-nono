...
fn run_verify(matches: &clap::ArgMatches) {
    // TODO: run `cargo build` with piped output before using escargot to show build progress
    // TODO: correct arguments
    let build_result = escargot::CargoBuild::new().exec().unwrap();
    let decoded_messages = build_result
        .into_iter()
        .filter_map(|raw_msg| raw_msg.map(|msg| msg.decode().ok()).ok())
        .collect::<Vec<_>>();
    // let artifact_messages = build_result
    // .into_iter()
    // .filter_map(|raw_msg| {
    // let raw_msg = raw_msg.unwrap();
    // let msg = raw_msg.decode();
    // if let Ok(escargot::format::Message::CompilerArtifact(artifact)) = msg {
