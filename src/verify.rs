...
fn main() {
    for path in env::args().skip(1) {
        ...
        // println!("{:?}", rlib_contains_namespace(&path.into(), "std"));
        // println!("{:?}", guess_dependencies_for_rmeta(&path.into()).unwrap());
    }
}
...
fn rlib_contains_namespace(rlib_path: &PathBuf, namespace_name: &str) -> bool {
    let contents = fs::read(&rlib_path).unwrap();
    let mut archive = goblin::archive::Archive::parse(&contents).unwrap();

    // let rlib_file = std::fs::File::open(rlib_path).unwrap();
    // let mut reader = std::io::BufReader::new(rlib_file);
